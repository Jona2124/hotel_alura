...
public class Busqueda extends JFrame {
	...
	public Busqueda() {
		...
		btnbuscar.addMouseListener(new MouseAdapter() {
		    @Override
		    public void mouseClicked(MouseEvent e) {
		        String idBuscar1 = txtBuscar.getText();
		        if (idBuscar1.isEmpty()) {
		            JOptionPane.showMessageDialog(null, "Por favor, ingrese un valor");
		        } else {
		            limpiarTabla();
		            LlenarTablaHuespedesId(idBuscar1);
	...
	private void LlenarTablaReservasId(String idBuscar) {
		List<Reserva> reservas = BuscarReservasId();
		if (reservas.isEmpty()) {
			JOptionPane.showMessageDialog(null, "No se encontraron reservas con el ID especificado");
			LlenarTablaReservas();
	...
	private void LlenarTablaHuespedesId(String idBuscar1) {
		List<Huespedes> huespedes = BuscarHuespedesId();
		if (huespedes.isEmpty()) {
			JOptionPane.showMessageDialog(null, "No se encontraron huéspedes con el ID especificado");
			LlenarTablaHuespedes();
		} else {
	...
	private void ActualizarReservas() {
	    Optional.ofNullable(modelo.getValueAt(tbReservas.getSelectedRow(), tbReservas.getSelectedColumn()))
	            .ifPresentOrElse(fila -> {
	                ...
	                if (tbReservas.getSelectedColumn() == 0 || tbReservas.getSelectedColumn() == 6) {
	                    JOptionPane.showMessageDialog(this, "No se pueden modificar los ID");
	                    LlenarTablaReservasId(); // Cargar nuevamente la información de la base de datos

	                    // Restaurar el ID anterior en la celda correspondiente
	                    modelo.setValueAt(id, tbReservas.getSelectedRow(), 0);
	                } else {
	    ...
	    Optional.ofNullable(modeloHuesped.getValueAt(tbHuespedes.getSelectedRow(), tbHuespedes.getSelectedColumn()))
	            .ifPresentOrElse(filaHuesped -> {
	                ...
	                if (tbHuespedes.getSelectedColumn() == 0 || tbHuespedes.getSelectedColumn() == 6) {
	                    JOptionPane.showMessageDialog(this, "No se pueden modificar los ID");
	                    LlenarTablaHuespedesId(); // Cargar nuevamente la información de la base de datos
	                    // Restaurar el ID anterior en la celda correspondiente
	                    modeloHuesped.setValueAt(id, tbHuespedes.getSelectedRow(), 0);
	                } else {
